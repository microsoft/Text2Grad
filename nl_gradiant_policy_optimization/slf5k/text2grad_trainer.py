...
from accelerate.utils import ProjectConfiguration, is_deepspeed_available, is_npu_available, is_xpu_available
...
class TEXT2GRADTrainer(BaseTrainer):
    ...
    def get_suggested_lengths(self, masks):
        ...
        for k, mask in enumerate(masks):
            ...
            for i in range(start_index, mask.numel()):
                if mask[i].item() == 1:
                    current_length += 1
                else:
                    if current_length > 0:
            ...
            if len(segment_lengths) > 0:
                ...
                median_length = (sorted_lengths[mid] + sorted_lengths[~mid]) / 2 if len(sorted_lengths) % 2 == 0 else \
                    sorted_lengths[mid]
            else:
                median_length = 0
    ...
    def get_length_weight_penalty(self, alpha, suggested_lengths, scores):
        lwps = []
        scores_penaled = []
        for k, score in enumerate(scores):
            ...
            for i in range(start_index, score_tensor.numel()):
                ...
                lwp.append(lwp_len)
                score_penaled.append(score_tensor[i].item() * lwp_len)
            lwps.append(lwp)
            scores_penaled.append(score_penaled)
        return lwps, scores_penaled
    ...
    def step(
            ...
            for backward_batch_start in range(0, bs, self.config.backward_batch_size):
                ...
                for mini_batch_start in range(0, self.config.backward_batch_size,
                    ...
                    with self.accelerator.accumulate(self.model):
                        ...
                        logprobs, logits, vpreds, _, all_tokens = self.batched_forward_pass(
                            self.model,
        ...
        return stats, sum(loss_ps) / len(loss_ps), sum(loss_vs) / len(loss_vs), sum(sum(advantages)) / (
                advantages.shape[0] * advantages.shape[1])
    ...
    def rematch_scores(self, scores, words, all_tokens):
        ...
        for i, token_list in enumerate(answers_tokens):
            ...
            total_skip_words_nums.append(float(total_skip_words_num / len(word_list)))
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        for i, scores in enumerate(data["scores"]):
            mask_i = mask[i]
            mask_score = [score for j, score in enumerate(scores) if mask_i[j] == 1]
